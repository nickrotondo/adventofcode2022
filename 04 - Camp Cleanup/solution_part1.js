...
function findRedundantAssignments(sectionAssignmentGroups) {
  let count = 0;

  sectionAssignmentGroups.forEach((sectionAssignments) => {
    if (
      (sectionAssignments[0][0] <= sectionAssignments[1][0] &&
        sectionAssignments[0][1] >= sectionAssignments[1][1]) ||
      (sectionAssignments[1][0] <= sectionAssignments[0][0] &&
        sectionAssignments[1][1] >= sectionAssignments[0][1])
    ) {
      count++;
    }
  });
...
console.log(findRedundantAssignments(sectionAssignmentGroups));
